...
class Services extends CI_Controller {
	...
	public function create()
	{
		if(isset($_POST['submit'])){
			...
			if($id=='')
			{
				$count =$this->db->where('name', $eduname)->count_all_results('master_services');
				if($count>0)
    			{
			...
			{
				$count =$this->db->where('name', $eduname)->where_not_in('id',$id)->count_all_results('master_services');
				if($count>0)
    			{
    				$msg="<div class='alert alert-danger'><strong>Failed!</strong> This keyword is already exists.</div>";
    				$this->session->set_flashdata('flashmsg',$msg);
	...
	public function delete()
	{
		...
		if($this->servicesmodel->delete($uid))
		{
			echo "Y";
		}
	}
