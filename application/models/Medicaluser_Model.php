...
class Medicaluser_Model extends CI_Model {
    ...
    public function changepass($userid){
        ...
        {			
			...
			$newpass=$this->input->post('pass');
				$this->db->where('USERID',$userid)->set('PASSWORD',md5($newpass))->update('chemistlogin');
       ...
       // $this -> db -> where('STATUS', '1');
        ...
        {			
			...
			return 'SUCCESS';
				...
				$otp=rand(100000,999999);
				$this->db->where('USERID',$row->USERID)->set('OTP',$otp)->update('chemistlogin');
				$this->session->set_userdata('medicalsignupuserid', $row->USERID);
...
thank you for being a part of Upchar.";
	...
	public function verifyforgototp($userid,$otp){
       ...
       // $this -> db -> where('APPROVED', '1');
        ...
        {			
			$row = $query->row();
            if($row->OTP==$otp){
				$this->db->where('USERID',$userid)->set('OTP',null)->set('STATUS','1')->update('chemistlogin');
               /*  $this->session->set_userdata('medicaluserid', $row->USERID);
                $this->session->set_userdata('medicaluseremail', $row->EMAIL);				           
				$this->session->set_userdata('medicalusername', $row->FNAME); */
	...
	public function register(){
		...
		$countemail=$this->db->where('EMAIL',$email)->count_all_results('chemistlogin');
		$countmobile=$this->db->where('MOBILE',$mobile)->count_all_results('chemistlogin');
...
thank you for being a part of Upchar.";
			...
			if($mobile)
			$udata['MOBILE']=$mobile;
			if($this->db->insert('chemistlogin',$udata))
			{  
