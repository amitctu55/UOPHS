...
<?php $this->load->view("includes/header.php"); ?>
                        ...
                        <div class="col-sm-4 col-md-offset-4 borders">
                          <div class="box-header">Type OTP Number</div>
                            <div class="label_name">                 
                                <p></p>
                                   <span>Type OTP Number 6 Digit</span><form id='drsignupotpform' action='<?=base_url();?>Doctoruser/verifysignupotp' method='post'>
	...
	$('.drresendotp').click(function(e) {		
		...
		$.ajax({			
		type: "POST",			
		url: '<?=base_url();?>Doctoruser/resendsignupotp',//myform.attr('action'),			
		//data: myform.serialize(),			
		success: function( response ) {				
			response = JSON.parse(response);				
			if(response.status=='success'){										
