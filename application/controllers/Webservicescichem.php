...
class Webservicescichem extends CI_Controller {

	
	public function __construct(){
		parent::__construct();
		$this->load->model('Chemistuser_Model');
		//$this->load->library('Sm_lib');
		$this->output
        ->set_content_type('application/json');
		if($this->input->post('druserid'))
		$this->did=@$this->input->post('druserid');
	}
	...
	public function verifyforgototp(){
		...
		$login = $this->Chemistuser_Model->verifyforgototp($userid,$otp);
		if($login=='SUCCESS'){
	...
	public function changepass(){
		...
		$login = $this->Chemistuser_Model->changepass($userid);
		if($login=='SUCCESS'){
	...
	public function forgotpass(){
		$mobile =$this->input->post('mobile');
		$login = $this->Chemistuser_Model->forgotpass($mobile);
		if($login=='SUCCESS'){
	...
	public function resendotp(){
		$mobile =$this->input->post('mobile');
		$login = $this->Chemistuser_Model->resendotp($mobile);
		if($login=='SUCCESS'){
			$response=array('status'=>'success','msg'=>'OTP Sent To Registered Mobile');
		}else {
			$response=array('status'=>'failed','msg'=>'Something went wrong');
	...
	public function profile_fetch(){
		...
		$profile=$this->db->where('id',$this->did)->get_where('profile_dr',array())->result_array();
		...
		$response=array('status'=>'success','msg'=>'Successfully','profile'=>$profile,'spl'=>$data_spl,'qua'=>$data_qua);
		echo json_encode($response);
