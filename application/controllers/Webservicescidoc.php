...
class Webservicescidoc extends CI_Controller {
	...
	public function __construct(){
		...
		//print_r($_POST);
		//echo $this->input->post('druserid');
		if($this->input->post('druserid')!='')
		{
		  $this->did=$this->db->where('user_id',$this->input->post('druserid'))->get('profile_dr')->row()->id;//last_query();
		}/* else{echo 'NA';} */
	}
	...
	public function changepass(){
		...
		$login = $this->Doctoruser_Model->changepass($userid);
		if($login=='SUCCESS'){
	...
	public function forgotpass(){
	   ...
	   $mobile =$this->input->post('mobile');
		$login = $this->Doctoruser_Model->forgotpass($mobile);
		if($login=='SUCCESS'){
	...
	public function resendotp(){
		$mobile =$this->input->post('mobile');
		$login = $this->Doctoruser_Model->resendotp($mobile);
		if($login=='SUCCESS'){
			$response=array('status'=>'success','msg'=>'OTP Sent To Registered Mobile');
		}else {
			$response=array('status'=>'failed','msg'=>'Something went wrong');
	...
	public function verifyforgototp(){
		...
		$login = $this->Doctoruser_Model->verifyforgototp($userid,$otp);
		if($login=='SUCCESS'){
	...
	public function profile_upload(){
		...
		{	
			...
			$uploadregproof='dr_reg_proof_'.$rname.$date.'.'.$extsign;
		...
		if($this->input->post('specialisation')){
	...
	public function profile_fetch(){
		...
		$profile=$this->db->select('profile_dr.*,master_council.name as councilname,master_city.name as cityname ')->where('profile_dr.id',$this->did)->join('master_council','regd_council=master_council.id')->join('master_city','city=master_city.id')->get_where('profile_dr',array())->result_array();
		...
		$response=array('status'=>'success','msg'=>'Successfully','profile'=>$profile,'spl'=>$data_spl,'qua'=>$data_qua,'url'=>admin_url().'public/assets/upload/');
		echo json_encode($response);
	...
	public function managepractice()
	{
		...
		foreach($clinic as $p){
			$data[]=array('id'=>$p->practice_id,'name'=>$p->name,'address'=>$p->address,'city'=>getCityName($p->city));
		}
	...
	public function addclinic(){
		...
		if($countguggestedclinic){
			foreach($suggestedclinic as $clinic){ 
				$suggdata[]=array('id'=>$clinic->id,'name'=>$clinic->name,'address'=>$clinic->address,'city'=>$clinic->city);
			}
			//return $suggestedclinic;
			$response=array('status'=>'success','msg'=>'Suggested Clinic Please Select One','data'=>$suggdata);
		}else{
			...
			$response=array('status'=>'success','msg'=>'Clinic Added Successfully');
		}
	...
	public function addpractice(){
		...
		if($countguggestedclinic + $countguggestedhospital){
			foreach($suggestedclinic as $clinic){ 
			 $suggdata[]=array('id'=>$clinic->id,'type'=>'C','name'=>$clinic->name,'address'=>$clinic->address,'city'=>getCityName($clinic->city));
			}
			foreach($suggestedhospital as $clinic){ 
			 $suggdata[]=array('id'=>$clinic->id,'type'=>'H','name'=>$clinic->name,'address'=>$clinic->address,'city'=>getCityName($clinic->city));
			}
			//return $suggestedclinic;
			$response=array('status'=>'success','msg'=>'Select One','data'=>$suggdata);
		}else{
			
			$response=array('status'=>'failed','msg'=>'No Data');
		}
		$this->json_output($response);
	}
	public function linkpractice(){
		...
		if($count){
			...
			$response=array('status'=>'success','msg'=>'Already Linked!');
		}else{
			...
			$response=array('status'=>'success','msg'=>'Linked Successfully');
		}
	...
	public function claimclinic(){
		//$udata=array('clinic_type'=>$this->input->post('practicetype'));
		//$this->db->where('user_id',$this->did)->update('profile_dr',$udata);
		$clinicid=$this->db->post('clinicid');
		$count=$this->db->where(array('clinic_id'=>$clinicid,'did'=>$this->did))->count_all_results('clinic_claimed');
		if($count==0){
		$udata2=array('clinic_id'=>$clinicid,'did'=>$this->did,'status'=>'P','date'=>date('Y-m-d H:i:s'));
			$this->db->insert('clinic_claimed',$udata2);
		$response=array('status'=>'success','msg'=>'Claimed Successfully');	
		}else{
			$response=array('status'=>'falied','msg'=>'Already Claimed');	
		}
		$this->json_output($response);
	...
	public function hospitallist(){
		$hospital=$this->db->get_where('hospital', array('status'=>'1'))->result();
		...
		$response=array('status'=>'success','msg'=>'Listed Hospital Successfully','data'=>$data);
	...
	public function update_dr_time_fee(){
		$pid=($this->input->post('practiceid'));//check if loged in 
		...
		$this->db->where(array('user_id'=>$this->did,'id'=>$pid))->set('fee',$fee)->update('dr_practice');
		$this->db->query("DELETE `timing`,`timing_session` FROM `timing` INNER JOIN `timing_session`  ON timing_session.timing_id=timing.id WHERE user_id='".$this->did."' AND user_type='D' AND practice_id='".$pid."';");
		for($key2=0;$key2<$hiddenday;$key2++){
			...
			$timingdata=array('practice_id'=>$pid,'user_id'=>$this->did,'M'=>$mon[$key2],'T'=>$tue[$key2],	'W'=>$wed[$key2],'TH'=>$thu[$key2],	'F'=>$fri[$key2],	'SA'=>$sat[$key2],	'S'=>$sun[$key2],	'status'=>'1');
			...
			foreach($sessions as $key3=>$value){
				if($from[$key2][$key3]=='' || $from[$key2][$key3]=='')
					continue;
				$sessiondata = array('timing_id'=>$tid,'from_timing'=>$from[$key2][$key3],'max_patient'=>$max_patient[$key2][$key3],'to_timing'=>$to[$key2][$key3],'status'=>'1');
