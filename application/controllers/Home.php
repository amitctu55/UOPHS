...
class Home extends CI_Controller {
	...
	public function index()
	{
		...
		$data['doctor_slid']=$this->db->order_by('id','RANDOM')->get_where('profile_dr',array('approved'=>'1','verified'=>'1'))->result();
		
		$data['image'] = $this->db->order_by('id','RANDOM')->limit('4')->get_where('gallery',array('id'))->result();
		$data['hospital'] = $this->db->order_by('id','RANDOM')->limit('1')->get_where('hospital',array('approved'=>'1','verified'=>'1'))->result();
        $data['userlogin']=$this->Userlogin_Model->c_count();
	...
	public function hospital()
{
    ...
    $data['gallery']=$this->db->get_where('hospitalgallery',array('uid'=>$id))->result();	
...
public function change_password()
        ...
        if($passwd->PASSWORD == $cur_password)
        {
            if($new_password == $conf_password)
            {
                ...
                {
                    $flashmsg="<div class='alert alert-success'><h4>Password changed Successfully!</h4></div>";
                    
                    //$flashmsg='Password Updated Successfully!';
						$this->session->set_flashdata('msg',$flashmsg);
                }
                else{
                    $flashmsg="<div class='alert alert-danger'><h4>Failed to changed Password</h4></div>";
